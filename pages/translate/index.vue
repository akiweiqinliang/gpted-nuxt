<template>
  <div id="translatePage">
    ...
    <Menu mode="horizontal" theme="light" class="floatMainMenu">
      ...
      <div class="nav">
        <Row justify="space-between" align="middle">
          <Row align="middle">
            <Row class="TABtnGroup">
              <div class="A" :class="{'active' : translateStatus === 'A' }" @click="translateStatus = 'A'">A</div>
              <div class="T" :class="{'active' : translateStatus === 'T' }" @click="translateStatus = 'T'">T</div>
              <div class="TA" :class="{'active' : translateStatus === 'TA' }" @click="translateStatus = 'TA'">T/A</div>
            </Row>
    ...
    <div class="mainContainer">
      ...
      class="fullWidth" :class="{
        ...
        <Row class-name="docDetail" :class="{'swap': swap}">
            ...
            <Col v-show="translateStatus === 'T' || translateStatus === 'TA'" flex="1">
              ...
              <TranslateProgressMask v-else-if="translating" @on-finish="translating = false"/>
              <div v-else>
      ...
      <AiDialogBox ref="relativeDialogBox" position-type="relative" @on-close="dialogOpenStatus = false"/>
    </div>
    <!--  -----------  内容 end ---------- -->
    <!--  ----------  ai对话框 start ------- -->
    ...
    <AiDialogBox ref="fixedDialogBox" @on-close="dialogOpenStatus = false"/>
    <!--  ----------  返回顶部 + AI悬浮按钮 end --------- -->
    <!--  ---------- 登录弹窗 start --------- -->
    <Drawer
...
export default {
  name: "Index",
  components: {AiDialogBox, MyBackTop},
  layout: 'translate',
  data() {
    return {
  ...
  watch: {
    translateStatus(val) {
      if (val === 'TA' && this.$refs.relativeDialogBox.showDrawer) {
        this.$refs.relativeDialogBox.close()
        this.$refs.fixedDialogBox.open()
      }
      if (val !== 'TA' && this.$refs.fixedDialogBox.showDrawer) {
        this.$refs.fixedDialogBox.close()
        this.$refs.relativeDialogBox.open()
      }
    }
  },
  created() {
    this.tenderId = Number(this.$route.query.tenderId)
    this.docId = Number(this.$route.query.docId)
    this.fetchData(this.tenderId,this.docId)
  },
  methods: {
    // 根据翻译选择状态打开ai对话框
    openAiDialogBox() {
      this.dialogOpenStatus = true
      if (this.translateStatus === 'TA') {
        this.$refs.fixedDialogBox.open()
      }else {
        this.$refs.relativeDialogBox.open()
      }
    ...
    // 获取文档
    async fetchData(tenderId, docId) {
      const res = await this.$axios.get(`/getTenderById`, {
        params: {
          tenderId
        }
      });
      this.docData = res.data.tender.docs.find(item => item.id === docId);
    },
    // 文档翻译
